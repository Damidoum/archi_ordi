...
- **Version Float (`main_float`)** : C'est la version de base demandée pour des vecteurs en simple précision. On a remarqué que le résultat AVX diffère légèrement du scalaire sur $1024^2$ éléments. C'est dû à l'accumulation dans les registres AVX 32 bits qui saturent leur précision (erreur d'absorption).
- **Version Double (`main_double`)** : On a fait une version en double précision pour corriger ce problème. Les résultats sont alors identiques, mais on ne traite que 4 éléments par registre AVX au lieu de 8, ce qui est moins performant.
